...
describe('Event tests', () => {
  it('getEventData should have correct value', () => {
    const event = new Event(JSON.stringify({data: 'testdata'}));
  ...
  it('getEventCount should have correct value and increment for every new instance', () => {
    let event = new Event(JSON.stringify({data: 'testdata1'}));
    expect(event.getEventCount()).to.equal(1);
    new Event(JSON.stringify({data: 'testdata2'}));
    expect(event.getEventCount()).to.equal(2);
    new Event(JSON.stringify({data: 'testdata3'}));
    event = new Event(JSON.stringify({data: 'testdata4'}));
  ...
  it('getUserId should have correct value when set', () => {
    const event = new Event(JSON.stringify({data: 'testdata'}), 1234);
